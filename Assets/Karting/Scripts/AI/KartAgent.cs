...
#region Rewards
        ...
        [Tooltip("Penalize when the agent drives pass checkpoints backwards")]
        public float BackwardsCheckpointPenalty;
        #endregion
...
#region Debugging
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            Debug.Log("Passing Checkpoint " + index + ", m_CkptIdx = " + m_CheckpointIndex);
            ...
            if (triggered > 0 && index > m_CheckpointIndex || index == 0 && m_CheckpointIndex == Colliders.Length - 1)
            {
                ...
                // penalize if kart passing checkpoint backwards
                AddReward(BackwardsCheckpointPenalty);
            }
        }
        ...
        public override void CollectObservations(VectorSensor sensor)
        {
            ...
            // Debug.Log("Collider Len: " + Colliders.Length);
        ...
        public override void OnActionReceived(ActionBuffers actions)
        {
            ...
            // Debug.Log("nextReward = " + reward);

            if (ShowRaycasts) Debug.DrawRay(AgentSensorTransform.position, m_Kart.Rigidbody.velocity, Color.blue);

            // Penalize for driving toward previous checkpoint
            // var prev = (m_CheckpointIndex - 1 + Colliders.Length) % Colliders.Length;
            // var prevCollider = Colliders[prev];
            // var prevDirection = (prevCollider.transform.position - m_Kart.transform.position).normalized;
            // var prevReward = Vector3.Dot(m_Kart.Rigidbody.velocity.normalized, prevDirection);
            // // Debug.Log("Prev = " + prev);
            // // Debug.Log("PrevReward = " + prevReward);
            // AddReward(prevReward * BackwardsCheckpointPenalty);

            // Add rewards if the agent is heading in the right direction
            AddReward(reward * TowardsCheckpointReward);
            AddReward((m_Acceleration && !m_Brake ? 1.0f : 0.0f) * AccelerationReward);
            AddReward(m_Kart.LocalSpeed() * SpeedReward * Sign(reward));
        ...
        public override void OnEpisodeBegin()
        {
            switch (Mode)
            {
                case AgentMode.Training:
                    ...
                    // fix start position at checkpoint 0
                    m_CheckpointIndex = 0;
