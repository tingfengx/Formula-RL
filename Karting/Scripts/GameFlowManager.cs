...
public class GameFlowManager : MonoBehaviour
{
    ...
    void StartRace() {
        ...
        {
            k.SetCanMove(true);
            var agent = k.GetComponent("KartAgent") as Agent;
            if (agent == null) // only load model to ml-agent cart, some carts can only be driven by humans
            {
                continue;
            }
            ...
            try
            {
                var nnModel = GetModelForBehaviorName(paths[i]);
                i++;
                agent.SetModel("Override_ArcadeKart", nnModel);
            }
            catch (Exception e)
            {
                string overrideError = $"Exception calling Agent.SetModel: {e}";
                Debug.Log(overrideError);
            }
    ...
    public NNModel GetModelForBehaviorName(string modelPath)
    {
        byte[] rawModel = null;
        ...
        try
        {
            rawModel = File.ReadAllBytes(modelPath);
            assetName = "Override - " + modelPath;
            // Debug.Log(assetName);
        }
        catch (IOException)
        {
        ...
        var asset = LoadOnnxModel(rawModel);
