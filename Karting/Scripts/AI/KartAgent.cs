...
#region Debugging
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            // MODIFICATION: Ensure that the agent touched the checkpoint and the new index is exatcly one larger than the m_CheckpointIndex. 

            // Debug.Log("m_CheckpointIndex: " + m_CheckpointIndex);
            if (triggered > 0 && index == m_CheckpointIndex + 1 || index == 0 && m_CheckpointIndex == Colliders.Length - 1)
            {
        ...
        public override void OnEpisodeBegin()
        {
            switch (Mode)
            {
                case AgentMode.Training:
                    ...
                    // always used the first index to simulate how to act with opponent cars in a match
                    m_CheckpointIndex = 0; 
